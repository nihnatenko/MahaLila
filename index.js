...
const textValue = ['Глубинно ощутить свою ценность мне помогло осознание, что самое ценное - это моя Жизнь. Это было бы невозможно без двух проводников. И это мои мама и папа. Большую часть своей сознательной жизни я испытывал сожаления, обиды и ожидания к ним. Не понимая, какой дар я получил через них. Жизнь сама по себе - это огромная ценность. Она наполнена безграничными возможностями и красотой. И это самое большое благо, которое ОНИ мне УЖЕ дали. На обиды и недовольство нет времени. Жизнь очень коротка. Лучше поскорее начни жить ее наполнено и радостно. А это возможно, только, ощутив ее ценность. Отпустив все претензии к родителям с глубокой благодарностью за их дар. \n' +
'Помогло мне в этом перепроживание момента своего рождения. Рождение - это один из основных сакральных моментов в Жизни Души, который закладывает основы для всей последующей Жизни. Ощутив это 1 раз - я закрепил это навсегда. Приглашаю пройти медитацию, чтобы перепрожить свое рождение заново прямо сейчас.\n',
...
const textDonat = '5169 3305 1922 0138\n' +
    'ФОП Плотніков Б.О.\n' +
...
bot.onText(/\pay/, msg => {
    ...
    setTimeout(() => {bot.sendMessage(msg.chat.id, textDonat); }, 2000);
    setTimeout(() => {bot.sendMessage(msg.chat.id, textThanks[0]); }, 15000);
});
...
bot.on('callback_query', (query) => {
    ...
    if (query.data === 'donat') {
        ...
        setTimeout(() => {bot.sendMessage(chatId, textDonat); }, 2000);
        setTimeout(() => {bot.sendMessage(chatId, textThanks[0]); }, 15000);
    }
    if (query.data === 'safer') {
